...
'''
    # 末尾のif __name__を見つけて直前に挿入
    idx = app_content.rfind('if __name__')
    if idx == -1:
        print("❌ if __name__ が見つかりません")
    else:
        app_content = app_content[:idx] + pdf_route + app_content[idx:]
...
with open(app_path, "r", encoding="utf-8") as f:
    check = f.read()
print("export_pdf in app.py:", "export_pdf" in check)
